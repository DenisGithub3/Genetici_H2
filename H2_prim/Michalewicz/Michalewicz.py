...
class BinaryGA:
    ...
    def binary_to_gray(self, bitstrings: np.ndarray) -> np.ndarray:
        """Convert binary bitstrings to Gray code."""
        gray = bitstrings.copy()
        gray[:,1:] = bitstrings[:,1:] ^ bitstrings[:,:-1]
        return gray

    def gray_to_binary(self, gray: np.ndarray) -> np.ndarray:
        """Convert Gray code bitstrings to binary code."""
        binary = gray.copy()
    ...
    def create_population(self, pop_size: int) -> np.ndarray:
        """Generate initial population as a 2D NumPy array of Gray-coded bitstrings."""
        # Generate random binary bitstrings
        random_bits = np.random.randint(0, 2, (pop_size, self.string_length))
        # Convert binary bitstrings to Gray code
        gray_codes = self.binary_to_gray(random_bits)
        return gray_codes

    def decode(self, bitstrings: np.ndarray) -> np.ndarray:
        """Decode Gray-coded bitstrings to real values."""
        # Convert Gray code to binary code
        binary_bitstrings = self.gray_to_binary(bitstrings)
        # Reshape binary_bitstrings to (pop_size, n_params, bits_per_param)
        pop_size = binary_bitstrings.shape[0]
        bit_array = binary_bitstrings.reshape(pop_size, self.n_params, self.bits_per_param)
